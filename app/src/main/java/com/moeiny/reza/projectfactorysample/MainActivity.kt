...
//            element.id= elements[i].decision_id
       ...
       txt_next.setOnClickListener {
           for (i in 0..selected.size-1){
               var item=viewModel.findDecisionByName(selected[i])
               if (item!=null){
    ...
    companion object{
        var elements=ArrayList<DecisionEntity>() as List<DecisionEntity>
